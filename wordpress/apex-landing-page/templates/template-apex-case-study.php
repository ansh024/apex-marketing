...
// No default track: this video has captions burned into the picture, so a
// <track> would render a second set over them. The field is still there for
// case studies whose video does not.
$captions   = $video ? apex_cs_media_url( apex_cs_field( 'case_captions' ), '' ) : '';
...
<section class="cs-client-story cs-shell" id="client-story">
  ...
  <div class="cs-client-story__grid">
    <div class="cs-video<?php echo $video ? '' : ' cs-video--poster'; ?>"><?php if ( $video ) : ?><video preload="metadata" playsinline poster="<?php echo esc_url( $poster ); ?>" src="<?php echo esc_url( $video ); ?>"><?php if ( $captions ) : ?><track kind="captions" src="<?php echo esc_url( $captions ); ?>" srclang="en" label="English" default><?php endif; ?></video><?php else : ?><img src="<?php echo esc_url( $poster ); ?>" alt="<?php echo esc_attr( sprintf( '%s, %s', apex_cs_field( 'case_testimonial_name', 'Arthur' ), apex_cs_field( 'case_testimonial_role', 'President & CEO' ) ) ); ?>"><?php endif; ?><?php if ( $video ) : ?><button type="button" class="cs-video__play" aria-label="<?php echo esc_attr( sprintf( 'Play %s’s testimonial', apex_cs_field( 'case_testimonial_name', 'Arthur' ) ) ); ?>">▶</button><?php endif; ?><div class="cs-video__caption"><strong><?php echo esc_html( apex_cs_field( 'case_testimonial_name', 'Arthur' ) ); ?></strong><span><?php echo esc_html( apex_cs_field( 'case_testimonial_role', 'President & CEO' ) ); ?></span></div></div>
